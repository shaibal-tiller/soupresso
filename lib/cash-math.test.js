...
test('computeCashSummary: baki given today counts as sales but not cash', () => {
  const s = computeCashSummary({
    totalCounted: 5000, openingBhangti: 500,
    bazarAdvanceReceived: 1000, bazarActualCost: 1000,
    nextBazarAdvance: 1000, nextBhangti: 500,
  ...
  assert.equal(s.totalSales, 4800);      // 5000 - 500 + 0 - 0 + 300 -- the 300 never touched the box
  assert.equal(s.cashTakenHome, 3500);   // unchanged -- baki given has no cash to take home
});

test('computeCashSummary: baki received today counts as cash but not today\'s sales', () => {
  const s = computeCashSummary({
    totalCounted: 5300, openingBhangti: 500, // the extra 300 in the box is the collected baki
    bazarAdvanceReceived: 1000, bazarActualCost: 1000,
    nextBazarAdvance: 1000, nextBhangti: 500,
    bakiReceived: 300,
  });
  assert.equal(s.totalSales, 4500);      // 5300 - 500 + 0 - 300 + 0 -- back out to the same sales as if it were never collected
  assert.equal(s.cashTakenHome, 3800);   // 5300 - 0 - 1000 - 500 -- the collected cash IS available to take home
});

test('computeCashSummary: baki given and received the same day net out on sales', () => {
  const s = computeCashSummary({
    totalCounted: 5200, openingBhangti: 500,
    bazarAdvanceReceived: 1000, bazarActualCost: 1000,
    nextBazarAdvance: 1000, nextBhangti: 500,
    bakiGiven: 150, bakiReceived: 200,
  });
  assert.equal(s.totalSales, 4650);      // 5200 - 500 + 0 - 200 + 150
  assert.equal(s.cashTakenHome, 3700);   // 5200 - 0 - 1000 - 500
});
