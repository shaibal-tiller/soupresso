...
// Server-safe "today" in the business's own timezone, independent of the
// server machine's timezone (Vercel functions run UTC, but the shop is
// Asia/Dhaka) — used where the client's local `todayStr()` isn't available.
export function todayStrTZ(timeZone = 'Asia/Dhaka') {
  return new Date().toLocaleDateString('en-CA', { timeZone }); // en-CA => YYYY-MM-DD
}

// Daily entries older than this many days (from `todayStrValue`) are locked
// in the app UI — corrections further back go through SQL directly.
export const ENTRY_EDIT_WINDOW_DAYS = 3;

// Temporary: lock suspended through this date at the user's request (2026-09-24
// session) — every entry is editable regardless of age while "today" is on or
// before this date. Resumes automatically the day after; remove this once it's
// no longer needed.
const EDIT_LOCK_SUSPENDED_THROUGH = '2026-09-24';

export function isEntryEditable(dateStr, todayStrValue) {
  if (todayStrValue <= EDIT_LOCK_SUSPENDED_THROUGH) return true;
  return dateStr >= shiftDateStr(todayStrValue, -ENTRY_EDIT_WINDOW_DAYS);
}

// Sunday-start week containing `dateStr` (matches the calendar picker's
// weekday columns), and the calendar month containing it — used for the
// sales-report "This week" / "This month" quick-picks.
...
export function endOfWeekStr(dateStr) {
  return shiftDateStr(startOfWeekStr(dateStr), 6);
}
...
export function endOfMonthStr(dateStr) {
  const [y, m] = dateStr.slice(0, 7).split('-').map(Number);
  return toDateStr(new Date(y, m, 0)); // day 0 of next month = last day of this month
}
